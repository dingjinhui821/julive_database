...
set spark.app.name=fact_event_esf_hour_dtl;
...
-- 指定解析项目集合 
and cast(get_json_object(t1.json,'$.project_id') as int) =31
or cast(floor(get_json_object(t1.json,'$.properties.product_id'))as decimal) in (8,801,807,808)
-- 剔除event null值数据
...
insert overwrite table julive_fact.fact_event_esf_hour_dtl partition(pdate,pplatform,phour) 
...
'all' as pplatform,
split(${hiveconf:etl_hour},"-")[1] as phour  

from tmp_event_user_merged tmp1 
left join ( -- 找到 触发 黑名单数据 的 distinct_id 
