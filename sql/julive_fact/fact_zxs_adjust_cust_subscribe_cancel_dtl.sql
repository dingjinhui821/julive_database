...
set spark.app.name=fact_zxs_adjust_cust_subscribe_cancel_dtl;
set spark.yarn.queue=etl;

insert overwrite table julive_fact.fact_zxs_adjust_cust_subscribe_cancel_base_dtl 
...
-- 指标:收入 
if(t2.status in (2) and t2.subscribe_type in (1,4),t3.forecast_payment_total * t1.value,0)   as adjust_subscribe_cancel_contains_ext_income, -- 核算退认购佣金-含外联(佣金) 
-- 指标:量 
if(t2.status in (2) and t2.subscribe_type in (1,4),t1.value,0)                               as adjust_subscribe_cancel_contains_ext_num, -- 核算退认购量-含外联 

to_date(from_unixtime(t3.back_datetime))                                                     as back_date,
case 
...
join ods.yw_subscribe t2 on t1.subscribe_id = t2.id 
left join ods.yw_deal t3 on t2.deal_id = t3.id 
left join julive_dim.dim_consultant_info t4 on t1.employee_id = t4.emp_id and from_unixtime(t3.back_datetime,'yyyyMMdd') = t4.pdate 
left join julive_dim.dim_consultant_info t5 on t1.manager_id = t5.emp_id and from_unixtime(t3.back_datetime,'yyyyMMdd') = t5.pdate 
...
insert overwrite table julive_fact.fact_zxs_adjust_cust_subscribe_cancel_dtl
select * from julive_fact.fact_zxs_adjust_cust_subscribe_cancel_base_dtl
where from_source =1;
insert overwrite table julive_fact.fact_wlmq_zxs_adjust_cust_subscribe_cancel_dtl
select * from julive_fact.fact_zxs_adjust_cust_subscribe_cancel_base_dtl
where from_source =2;
insert overwrite table julive_fact.fact_esf_zxs_adjust_cust_subscribe_cancel_dtl
select * from julive_fact.fact_zxs_adjust_cust_subscribe_cancel_base_dtl
where from_source =3;
insert overwrite table julive_fact.fact_jms_zxs_adjust_cust_subscribe_cancel_dtl
select * from julive_fact.fact_zxs_adjust_cust_subscribe_cancel_base_dtl
where from_source =4;
insert into table julive_fact.fact_jms_zxs_adjust_cust_subscribe_cancel_dtl
select * from julive_fact.fact_zxs_adjust_cust_subscribe_cancel_base_dtl
