...
set spark.app.name=dim_sougou_keyword_info;  
...
drop table if exists tmp_dev_1.tmp_dim_sougou_keyword_info;
  create table tmp_dev_1.tmp_dim_sougou_keyword_info as
  SELECT
      *
  FROM julive_dim.dim_sougou_keyword_info
  WHERE pdate = from_unixtime(unix_timestamp(date_add(from_unixtime(unix_timestamp('${DATE_ID}','yyyyMMdd')),-1),'yyyy-MM-dd'),'yyyyMMdd');

INSERT overwrite TABLE julive_dim.dim_sougou_keyword_info partition(pdate = '${DATE_ID}')
select 
tmp.skey,            
tmp.create_date,
tmp.media_id,        
...
SELECT if(src_keyword_baseinfo.keyword_id IS NOT NULL,src_keyword_baseinfo.keyword_id,tmp_dim_keyword_info.keyword_id) AS skey ,
   ...
   FROM
     (SELECT media_id ,
             ...
             wmatchprefer ,
             pause
      FROM tmp_dev_1.tmp_dim_sougou_keyword_info)tmp_dim_keyword_info
   FULL JOIN
     (SELECT pdate AS create_date ,
             ...
             if(match_type = '0','10', 
               if(match_type = '2','23', 
                 if(match_type = '1','30', 
                   if(match_type = '5','21', 
                     if(match_type = '4','22',NULL))))) AS match_type ,
             if(match_type = '0','10-精确匹配',
               if(match_type = '2','23-短语核心包含', 
                 if(match_type = '1','30-广泛匹配', 
                   if(match_type = '5','21-短语同义包含', 
                     if(match_type = '4','22-短语精确包含',NULL))))) AS match_type_desc ,
             if(status = '31','141',
               if(status = '32','42',
                 if(status = '33','46',
                   if(status = '34','44',
                     if(status = '35','41',
                       if(status = '36','49',
                         if(status = '37','50',NULL))))))) AS status ,
             if(status = '35','正常', 
               if(status = '32','暂停', 
                 if(status = '34','搜索无效', 
                   if(status = '33','审核中', 
                      if(status = '36','计算机搜索无效', 
                        if(status = '37','移动搜索无效',
                          if(status = '31','审核未通过',
                            if(status = '142','本地新增',NULL)))))))) AS status_desc ,
             if(pause = 'false',0,
              if(pause = 'true',1,NULL)) AS pause ,
             pc_url AS pc_url ,
             m_url AS m_url ,
             ...
             null AS m_reliable ,
             null AS m_reason
      FROM julive_ods.src_sougou_keyword_baseinfo
...
alter table julive_dim.dim_sougou_keyword_info drop partition (pdate<'${DATE_ID}');
