...
src_path=/data2/channel_360/day/${before_yes_date}
tar_path=/dw/julive_ods/src_360_keyword_baseinfo/${yes_date}
echo "源路径:${src_path}"
echo "目标路径:${tar_path}"
...
do
 ...
 hive -e "alter table julive_ods.src_qihoo_keyword_baseinfo add partition (pdate='${yes_date}',file_name='${file_path}') location '${tar_path}/${file_path}/'"
...
EXCODE=$?
 if [ $EXCODE -eq 0 ]; then
    ...
    echo "********************************FAILED WITH EXITCODE $EXCODE**********************************"
     ssh -p 10001 etl@test01 "python /data4/nfsdata/dingtalk_monitor/monitor/azkaban_responder.py -m \"@15738713772,julive_ods.dim_qihoo_keyword_infi_to_hdfs.sh FAILED\""
fi
