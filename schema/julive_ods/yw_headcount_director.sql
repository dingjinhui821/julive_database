drop table if exists ods.yw_headcount_director;
create external table ods.yw_headcount_director(
id                                                     int             comment '主键id',
city_id                                                int             comment '城市id',
past_city_id                                           int             comment '过去城市id',
...
need_attendance_days                                   double          comment '应出勤天数,取考勤表extra_need_attendance_days',
...
adjust_rest_days                                       double          comment '可调休天数(总)',
amend_leave_work_days                                  double          comment '修正旷工假',
amend_personal_leave_days                              double          comment '修正事假',
on_job_days                                            double          comment '在职天数',
adjust_days                                            double          comment '结算月天数',
head_count                                             double          comment '人头数(在职天数/结算月天数)',
...
adjust_past_city_id                                    bigint          comment '过去核算城市',
etl_time                                               string          comment 'ETL跑数时间'
) row format delimited fields terminated by '\001' 
lines terminated by '\n' 
stored as textfile  
location '/dw/ods/yw_headcount_director'
;
