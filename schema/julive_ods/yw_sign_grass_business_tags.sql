drop table if exists ods.yw_sign_grass_business_tags;
create external table ods.yw_sign_grass_business_tags(
id                                                     bigint          comment '',
...
sign_grass_id                                          bigint          comment '排号标签表',
label_type                                             int             comment '标签类型(1:预测标签2延迟标签3直接录入4:发送风险提示)',
label_value                                            int             comment '标签名（定义常量 201:未发送风险提示202:已发送风险提示）',
...
lines terminated by '\n' 
stored as textfile  
location '/dw/ods/yw_sign_grass_business_tags'
;
