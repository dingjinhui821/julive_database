drop table if exists ods.yw_paihao_business_tags;
create external table ods.yw_paihao_business_tags(
id                                                     bigint          comment '',
...
paihao_id                                              bigint          comment '排号标签表',
label_type                                             int             comment '标签类型(标签类型(1:预测标签2:延迟标签3:直接录入4:发送风险提示))',
label_value                                            int             comment '标签名（1:预测会认购2预测不会认购101:直接排号105:预判认购成功106:预判认购失败201:未发送风险提示202:已发送风险提示）',
...
lines terminated by '\n' 
stored as textfile  
location '/dw/ods/yw_paihao_business_tags'
;
