#!/bin/bash
 ...
 monitorTaskName="app_user_lifecycle_send_message_history"
 monitorGroup="etl"
 monitorPhone="15738713772"

 #2、配置任务信息：sql文件路径和日志文件路径（日志文件需要通过日期时间配置唯一文件名）
 sqlFile="/data4/nfsdata/julive_dw/etl/sql/julive_app/app_user_lifecycle_send_message_history.sql"
 logFile="/data4/nfsdata/julive_dw/etl/logs/julive_app/app_user_lifecycle_send_message_history${pre}_${dateID}.log"
