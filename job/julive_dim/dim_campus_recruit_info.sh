...
#2、配置任务信息：sql文件路径和日志文件路径（日志文件需要通过日期时间配置唯一文件名） _${RANDOM} 
sqlFile="/data4/nfsdata/julive_dw/etl/sql/julive_dim/dim_campus_recruit_info.sql"
logFile="/data4/nfsdata/julive_dw/etl/logs/julive_dim/dim_campus_recruit_info${pre}_${dateID}_${RANDOM}.log"
...
python /data4/nfsdata/dingtalk_monitor/monitor/azkaban_responder.py -m "@15738713772: etl表(dim_campus_recruit_info)ETL发生异常，请尽快处理!"
eeooff
    exit $EXCODE
fi
