...
outputDir=/dw/julive_dim/dict_budget_range
...
--query "select id,city_id,city_name,grade_id,grade_name,low_budget,high_budget,create_time,update_time,current_timestamp() as etl_time from julive_dw.dict_budget_range where \$CONDITIONS " \
--columns "id,city_id,city_name,grade_id,grade_name,low_budget,high_budget,create_time,update_time,etl_time" \
...
python /data4/nfsdata/dingtalk_monitor/monitor/azkaban_responder.py -m "@15738713772: ODS采集(dict_budget_range)ETL发生异常，请尽快处理!"
eeooff
    exit $excode
fi

jobVersion=$(date +%s)_$RANDOM
indiValue=$(hive -e "select count(1) from julive_dim.dict_budget_range;")
endTime=$(date +%s)
