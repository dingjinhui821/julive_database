...
if [[ $# == 2 ]];then
 ...
 monitorTaskName="fact_comjiaunique_final_version_dtl"
 monitorGroup="etl"
 monitorPhone="15738713772"

 #2、配置任务信息：sql文件路径和日志文件路径（日志文件需要通过日期时间配置唯一文件名）
 sqlFile="/data4/nfsdata/julive_dw/etl/sql/julive_fact/fact_comjiaunique_final_version_dtl.sql"
 logFile="/data4/nfsdata/julive_dw/etl/logs/julive_fact/fact_comjiaunique_final_version_dtl${pre}_${dateID}.log"
