...
sqlFile="/data4/nfsdata/julive_dw/etl/sql/julive_fact/fact_dsp_sem_keyword_report_day_dtl.sql"
logFile="/data4/nfsdata/julive_dw/etl/logs/julive_fact/fact_dsp_sem_keyword_report_day_dtl${pre}_${dateID}_${RANDOM}.log"
...
python /data4/nfsdata/dingtalk_monitor/monitor/azkaban_responder.py -m "@15998885390: etl表(fact_event_dtl)执行成功、执行成功、执行成功!"
eeooff

else
    echo "********************************FAILED WITH EXITCODE $EXCODE**********************************"
ssh -p10001 test01 <<eeooff
python /data4/nfsdata/dingtalk_monitor/monitor/azkaban_responder.py -m "@15998885390: etl表(fact_event_dtl)ETL发生异常，请尽快处理!"
eeooff
    exit $EXCODE
fi
